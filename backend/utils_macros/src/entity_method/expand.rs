use super::lifetime::{AddLifetimeToImplTrait, CollectLifetimes};
use super::receiver::{mut_pat, ReplaceSelf};
...
use syn::{
    parse_quote, parse_quote_spanned, Attribute, Block, FnArg, GenericParam, Ident, ImplItemFn,
    Pat, PatIdent, Receiver, ReturnType, Signature, Token, Type, WhereClause,
};
...
pub fn expand(ctx_arg_name: String, method: &mut ImplItemFn) {
    if method.sig.asyncness.is_none() {
        panic!("non async method can't be marked with #[entity_method]");
    }
    ...
    let ((ctx_arg_ident, ctx_arg_ty), ret_ty) = transform_sig(ctx_arg_name, sig);
    transform_block(ctx_arg_ident, ctx_arg_ty, ret_ty, original_sig, block);
...
//     is_local: bool,
// ) {
fn transform_sig(cxt_arg_name: String, sig: &mut Signature) -> ((Ident, Box<Type>), Box<Type>) {
    let default_span = sig.asyncness.take().unwrap().span;
    sig.fn_token.span = default_span;

    let (ret_arrow, ret_ty): (RArrow, Box<Type>) = match &sig.output {
        ReturnType::Default => (
            Token![->](default_span),
            parse_quote_spanned!(default_span=> ()),
        ),
        ReturnType::Type(arrow, ret) => (*arrow, parse_quote!(#ret)),
    };
        ...
        match param {
            GenericParam::Type(param) => {
                ...
                where_clause_or_default(&mut sig.generics.where_clause)
                    .predicates
                    .push(parse_quote_spanned!(span=> #param_name: 'entity_method + #bounds));
                // TODO
            }
            GenericParam::Lifetime(param) => {
                ...
                where_clause_or_default(&mut sig.generics.where_clause)
                    .predicates
                    .push(parse_quote_spanned!(span=>#param: 'entity_method + #bounds));
                // TODO
            }
    ...
    for elided in lifetimes.elided {
        ...
        where_clause_or_default(&mut sig.generics.where_clause)
            .predicates
            .push(parse_quote_spanned!(elided.span()=> #elided: 'entity_method));
        // TODO
    }

    sig.generics
        .params
        .push(parse_quote_spanned!(default_span=> 'entity_method_future));
    sig.generics
        .params
        .push(parse_quote_spanned!(default_span=> 'entity_method: 'entity_method_future));
    ...
    for (i, arg) in sig.inputs.iter_mut().enumerate() {
        match arg {
            ...
            FnArg::Typed(arg) => {
                match &*arg.pat {
                    Pat::Ident(arg_ident) if arg_ident.ident.to_string() == cxt_arg_name => {
                        ctx_arg = Some((arg_ident.ident.clone(), arg.ty.clone()));
                        continue;
                    }
                    _ => (),
                };
        ...
        fitlered_inputs.push(arg.clone());
    }

    let Some((ctx_arg_ident, ctx_arg_ty)) = ctx_arg else {
        panic!("context argument '{}' does not presented", cxt_arg_name);
    };

    sig.inputs = fitlered_inputs;

    sig.output = parse_quote_spanned! {default_span=>
        #ret_arrow ::utils::entity::method::EntityMethod<'entity_method_future, 'entity_method, #ctx_arg_ty, #ret_ty>
    };

    ((ctx_arg_ident, ctx_arg_ty), ret_ty)

    // let bounds = if is_local {
    //     quote_spanned!(default_span=> 'entity_method_future)
    // } else {
    //     quote_spanned!(default_span=> ::core::marker::Send + 'entity_method_future)
    // };
    // let bounds = quote_spanned!(default_span=> ::core::marker::Send + 'entity_method_future);
    ...
    // };
    // sig.output = parse_quote_spanned! {default_span=>
    //     #ret_arrow ::utils::entity::method::EntityMethod<>
    // };
}
...
fn transform_block(
    ...
    let body = quote_spanned! {block.brace_token.span=>
        let m: ::std::boxed::Box<
            dyn ::std::ops::FnOnce(#ctx_arg_ty) ->
                ::std::pin::Pin<::std::boxed::Box<dyn ::std::future::Future<Output = #ret_ty> + ::std::marker::Send + 'entity_method_future>>
                + ::std::marker::Send
                + ::std::marker::Sync
                + 'entity_method,
        > = ::std::boxed::Box::new(move |#ctx_arg_ident: #ctx_arg_ty| {
            #(#decls)*
        ...
        ::utils::entity::method::EntityMethod::new(m)
    };
    block.stmts = parse_quote!(#body);
