...
pub use transaction::TransactionModule;

pub struct DataModule<C> {
    config_module: Arc<C>,
    conn: sqlx::PgPool,
}

impl<C: ConfigModule + Clone> DataModule<C> {
    ...
    pub async fn begin_transaction(&self) -> Result<TransactionModule<C>, anyhow::Error> {
