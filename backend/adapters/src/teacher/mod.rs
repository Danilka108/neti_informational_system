...
impl PgTeacherRepo {
    ...
    async fn update(&self, entity: Entity) -> Result<Teachers, anyhow::Error> {
        ...
        query
            ...
            .and_where(Expr::col(TeachersIden::Id).eq(entity.id.value))
            .returning_all();
...
impl teacher::Repo for PgTeacherRepo {
    ...
    async fn find(&self, id: EntityId) -> Result<Option<Entity>, anyhow::Error> {
        let select = self
            .select(Expr::col((TeachersIden::Table, TeachersIden::Id)).eq(id.value))
            .await?;
    ...
    async fn find_by_person_id(
        ...
        let select = self
            .select(Expr::col((TeachersIden::Table, TeachersIden::PersonId)).eq(person_id.value))
            .await?;
